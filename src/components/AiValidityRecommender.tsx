...
export interface Recommendation {
  column: string;
  dimension: 'validity' | 'consistency' | 'uniqueness' | 'completeness';
...
const AiValidityRecommender = forwardRef<AiValidityRecommenderHandle, AiValidityRecommenderProps>(
  function AiValidityRecommender(
    ...
    // Completeness is auto-mirrored from consistency columns — show same columns
    const completenessRecs = consistencyRecs.map(r => ({ ...r, dimension: 'completeness' as const }));
      ...
      <div className="bg-white border border-violet-100 rounded-xl shadow-sm overflow-hidden">
        ...
        {!collapsed && (
          <>
            ...
            {/* Completeness section — auto-mirrored from consistency */}
            {completenessRecs.length > 0 && (
              <>
                <div className="mx-4 border-t border-slate-100" />
                <div className="p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {completenessRecs.map(rec => (
                      <div
                        key={rec.column}
                        className="p-4 border border-green-100 rounded-lg bg-green-50/40 hover:bg-green-50 transition space-y-2"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <span className="text-sm font-semibold text-slate-800 break-all">{rec.column}</span>
                          <span className="flex items-center gap-1 text-xs text-green-600 bg-green-50 px-1.5 py-0.5 rounded-full border border-green-200 whitespace-nowrap flex-shrink-0">
                            Completeness
                          </span>
                        </div>
                        <div className="text-xs font-medium text-green-700 bg-white px-2 py-1 rounded border border-green-100">
                          Check values are present and not empty
                        </div>
                        <p className="text-xs text-slate-500 leading-relaxed">Mirrored from Consistency — these columns should also be checked for missing values.</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="px-5 py-2.5 border-t border-violet-50 bg-violet-50/30 space-y-0.5">
              ...
              {consistencyRecs.length > 0 && (
                <p className="text-xs text-blue-500">
                  Consistency columns added — configure a reference dataset or inline list for each. Also auto-added to Completeness (no config needed).
                </p>
              )}
              {uniquenessRecs.length > 0 && (
